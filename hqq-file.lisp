  ...
  (let* ((begin-db (concatenate 'string
    ...
    (with-open-file (that path
      ...
      (loop for test-line = (read-line that nil 'conclude)
	    ...
	    if (and (> (length test-line) begin-db-len)
		    (string=
...
(defgeneric load-hqqs-from-file (path &optional initial)
  (:documentation "Load every hqq-database in a file into an array thereof."))
